...
#     Attributes
#     ----------
#     _pp: Piecewise polynomial representation of the collection of dense output
#     _neqs: Number of equations represented in the dense output
#     """
#     def __init__(self, rk_dense_out):
#         coef = self._convert_q_to_c(rk_dense_out.Q, rk_dense_out.h, rk_dense_out.y_old)
...
#             self._pp.append(PPoly(np.asarray([coef[i, :]]).T, xrange, extrapolate=False))
...
#         """
#         Internal function that converts the Q coefficients from ODE dense output to
#         the coefficients needed for a piecewise polynomial segment.
...
#         Notes
#         -----
#         * Q vector of cefficients is polynomial coefficients from x^1 to x^{order+1}
#         * The constant term of polynomial is stored in separate term, y0
#         * The dense output polynomial independent variable, x, always goes from zero
#           to one. This is different than piecewise polynomial by a factor of h^{n-1}
#           for each Q_n term.
#         """
#         ydim, pdim = Q.shape
#         pdim=pdim+1
