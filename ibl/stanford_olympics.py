...
class StanfordOlympics1968:
    ...
    class StationData:
        """
        Data that is reported for each station in flow.
        ...
        def __str__(self):
            ...
            strout += f"    dU_edx: {self.dU_edx}\n"
            strout += f"    delta_m: {self.delta_m}\n"
            strout += f"    H_d: {self.H_d}\n"
            strout += f"    H_k: {self.H_k}\n"
            strout += f"    G: {self.G}\n"
            strout += f"    c_f: {self.c_f}\n"
            strout += f"    c_f (Ludwieg-Tillman): {self.c_f_LT}\n"
            strout += f"    c_f (Reported): {self.c_f_E}\n"
            strout += f"    beta: {self.beta}\n"
    ...
    def change_case_data(self, case):
        ...
        case_filename = (f"{pyBL.__path__[0]}/../stanford_olympics/"
                         + f"1968/case {case}.txt")
        with open(case_filename, "r", encoding="utf8") as case_file:
    ...
    def velocity(self):
        """
        Return 3-tuple of edge conditions.

        This method returns the x positions, edge velocities, and rates of
        change of the edge velocity.
    ...
    def velocity_smooth(self):
        """
        Return 3-tuple of smoothed edge conditions.

        This method returns the x positions, smoothed edge velocities, and
    ...
    def c_f(self):
        """
        Calculate skin friction coefficient at each station.
    ...
    def c_f_LT(self):
        """
        Skin friction coefficient from Ludwieg-Tillman formula at each station.
