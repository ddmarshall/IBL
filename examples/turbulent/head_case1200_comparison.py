...
def compare_case1200() -> None:
    """Compare the Head method results to expiremental data."""
    so68 = StanfordOlympics1968("1200")
    x = so68.x()
    u_e = so68.u_e()
    du_e = so68.du_e()
    x_sm = so68.x_smooth()
    ...
    _ = ax.plot(x_ref, np.abs(1-temp/c_f_ref), color=head_sm2_color)
    _ = ax.set_ylabel("Relative Difference")
    _ = ax.set_ylim((1e-3,1))
    ax.set_yscale('log')
    ax.grid(True)
