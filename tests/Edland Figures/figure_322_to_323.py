...
# # -*- coding: utf-8 -*-
# """
# Created on Sat Aug  6 00:33:57 2022
...
#     try:
#         hsd_l = HeadSimData(xfoil_lower.s[lower_range],xfoil_lower.Ue[lower_range],U_inf,nu,float(michel_l.x_tr),theta0=float(ts_l.theta(michel_l.x_tr)), h0=michel_l.h0)
#         hs_l = HeadSim(hsd_l) 
#         while hs_l.status=='running':
#             hs_l.step()

#         # extract the laminar and turbulent results into arrays
#         michel_idx = np.where(xfoil_lower.s<=michel_l.x_tr)[0][-1]
#         lam_slice=slice(lower_range.start, michel_idx+1)
#         turb_slice=slice(michel_idx+1, lower_range.stop)
...
#     try:
#         hsd_u = HeadSimData(xfoil_upper.s[upper_range],xfoil_upper.Ue[upper_range],U_inf,nu,float(michel_u.x_tr),theta0=float(ts_u.theta(michel_u.x_tr)), h0=michel_u.h0)
#         hs_u = HeadSim(hsd_u) 
#         while hs_u.status=='running':
#             hs_u.step()

#         # extract the laminar and turbulent results into arrays
#         michel_idx = np.where(xfoil_upper.s<=michel_u.x_tr)[0][-1]
#         lam_slice=slice(upper_range.start, michel_idx+1)
#         turb_slice=slice(michel_idx+1, upper_range.stop)
