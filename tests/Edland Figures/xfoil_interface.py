...
def read_xfoil_dump_file(xfoil_filename: str,
                         U_inf: float = 1) -> Tuple[Tuple, Tuple, Tuple]:
    """
    ...
    PanelValues = namedtuple('PanelValues', ['s', 'x', 'y', 'Ue', 'delta_d',
                                             'delta_m', 'cf', 'H', 'Hstar',
                                             'P', 'm', 'K'])
    WakeValues = namedtuple('WakeValues', ['s', 'x', 'y', 'Ue', 'delta_d',
                                           'delta_m', 'H'])
    
    xfoil_file = open(xfoil_filename, 'r')
    ...
    panel_array = np.asarray(panel_terms)
    last_upper_idx = np.where(np.sign(panel_array[:-1, 3])
                              != np.sign(panel_array[1:, 3]))[0][0]
    s_af = panel_array[-1, 0]
    
    # interpolate stagnation point
    dU = panel_array[last_upper_idx+1, 3]-panel_array[last_upper_idx, 3]
    frac = -panel_array[last_upper_idx, 3]/dU
    stag_col = (frac*panel_array[last_upper_idx+1,:]
                + (1 - frac)*panel_array[last_upper_idx,:])
    ...
    panel_upper_terms = panel_array[last_upper_idx::-1, :]
    ...
    # lower panel
    panel_lower_terms = panel_array[last_upper_idx+1:, :]
    # insert the stagnation point
    ...
    wake_array = np.asarray(wake_terms)
    wake_array[:,0] = wake_array[:,0] - s_af
    # remove zeros from skin friction coefficient
    wake_array = np.delete(wake_array, 6, 1)
    wake_values = WakeValues(*wake_array.T)
